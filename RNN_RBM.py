...
class RnnRbm:
    ...
    def __init__(self, n_hidden=150, n_hidden_recurrent=100,lr=0.001,r=(21, 109), dt=0.3 ):
        ...
        (v, v_sample, cost, monitor, params, updates_train, v_t,
            updates_generate) = build_rnnrbm(r[1] - r[0], n_hidden, n_hidden_recurrent)
    ...
    def train(self, files, batch_size=100, num_epochs=200):
        ...
        try:
            for epoch in range(num_epochs):
                ...
                loss.append(np.abs(numpy.mean(costs))) 
                sys.stdout.flush()
...
def test_rnnrbm(batch_size=100, num_epochs=100):
    
    model = RnnRbm()
    path = 'Maestro2017/*.midi'
    # path = 'midi1/*.mid'
    Music_data =  glob(path)
    # model.train( ['2ndtime.mid'], batch_size=batch_size, num_epochs=num_epochs)
    model.train(Music_data, batch_size=batch_size, num_epochs=num_epochs)
    
    return model

if __name__ == '__main__':
    model = test_rnnrbm()
    t = r'Maestro2017Generated'
    songg = '/RNN_RBM_SampleFinal' 
    for i in range(10):
        model.generate(t + songg + str(i) + '.mid')
    # model.generate('2ndsampleee.mid')
    pylab.show()

# k = r'Maestro2017Generated/RNN_RBM_SampleFinal/RNN_RBM_SampleFinal0.mid'